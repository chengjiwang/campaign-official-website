...
export default function Policy() {
  return (
    <section id='policy' className='container max-w-screen-md mx-auto pt-6 md:pt-16'>
      <h2 className='text-center text-xl font-bold mb-4 md:mb-8 md:text-6xl'>政策議題</h2>
      {policyItem.map(item => {
        return (
          <div key={item.title} className='mb-6'>
            <h3 className='text-xl font-medium mb-2 px-8 md:mb-4 md:text-3xl md:text-center'>{item.title}</h3>
            <ol className='px-6'>
              {item.policies.map(policy => <li className='list-decimal list-inside mb-2' key={policy}>{policy}</li>)}                           
            </ol>
          </div>
        )
      })}
